...
const { StatusCodes } =require("http-status-codes");
    ...
    const addpremium=async(req,res)=>{
        ...
        if(plan_type=='' || total_cost=='' || plan_start_date=='' || plan_end_date=='' || payment_id=='' || message=='' || packageID==''){
            ...
            try {
                    ...
                    //const getcounteres=await SellerPremium.find(req.params.id)
                  /*   if(getcounteres.length > 0){
                        ...
                        const updateer=await SellerPremium.create(data)
                        if(updateer){
    ...
    const getPremiumbyUser=async(req, res)=> {
        try {
            ...
            const data = await SellerPremium.find({userID:userid})
   ...
   const deletePremium=async(req, res)=> {
    try {
        const premiumCheck=await SellerPremium.findById(req.params.id)
        if(premiumCheck){
