...
exports.getAll = async (req, res) => {
  try {
    const items = await Wishlist.find({ user_id: req.user._id }).sort({ createdAt: -1 });
...
exports.check = async (req, res) => {
  try {
    const exists = await Wishlist.exists({ user_id: req.user._id, item_id: req.params.item_id });
...
exports.getIds = async (req, res) => {
  try {
    const items = await Wishlist.find({ user_id: req.user._id }, 'item_id');
