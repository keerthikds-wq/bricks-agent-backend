...
// POST /api/builder-premiums/create-order
exports.createBuilderOrder = async (req, res) => {
    const { months, packageID } = req.body;
    if (!months || months < 1) {
        return res.status(400).json({ error: true, message: 'months is required' });
    }
    try {
        let amountPaise;
        if (packageID) {
        ...
        const order = await razorpay.orders.create({
            amount:   amountPaise,
            currency: 'INR',
            receipt:  'builder_' + req.user.id + '_' + Date.now(),
            notes: { builderId: req.user.id.toString(), months: months.toString(), type: 'builder_subscription' },
        });
        ...
        console.error('createBuilderOrder error:', err);
        return res.status(500).json({ error: true, message: err.message });
    }
};

// POST /api/builder-premiums/verify-payment
exports.verifyBuilderPayment = async (req, res) => {
    const { razorpay_order_id, razorpay_payment_id, razorpay_signature, months, packageID } = req.body;
    if (!razorpay_order_id || !razorpay_payment_id || !razorpay_signature) {
        return res.status(400).json({ error: true, message: 'Missing payment verification fields' });
    }
    if (!_verifySignature(razorpay_order_id, razorpay_payment_id, razorpay_signature)) {
        return res.status(401).json({ error: true, message: 'Payment verification failed -- invalid signature' });
    }
    try {
        const m       = Number(months) || 1;
        const now     = new Date();
        const endDate = new Date(now);
        endDate.setDate(endDate.getDate() + m * 30);
        const premium = await BuilderPremium.create({
            plan_type:       m + ' month' + (m > 1 ? 's' : ''),
            purchase_date:   now,
            total_cost:      req.body.amount ? req.body.amount / 100 : m * 399,
            plan_start_date: now,
            plan_end_date:   endDate,
            payment_id:      razorpay_payment_id,
            message:         'Your ' + m + '-month builder subscription is active!',
            userID:          req.user.id,
            packageID:       packageID || undefined,
        });
        return res.status(201).json({ error: false, message: 'Builder subscription activated successfully', data: premium });
    } catch (err) {
        console.error('verifyBuilderPayment error:', err);
        return res.status(500).json({ error: true, message: err.message });
    }
};
